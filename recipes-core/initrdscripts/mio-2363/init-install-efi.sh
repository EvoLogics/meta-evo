...
# remove modules from size, as they are not installed to /boot
if [ -e /run/media/$1/modules.tgz ]; then
    boot_size=$(( boot_size - $( du -ms /run/media/$1/modules.tgz | awk '{print $1}') ))
fi
# double and add 20M to provide some extra space for users and account
# for rounding in the above subtractions
boot_size=$(( 2 * boot_size + 20 ))

# 64M minimum size
boot_size=$(( 64 > boot_size ? 64 : boot_size ))
...
rootfs_size=$(( (disk_size-boot_size-swap_size) / 2))

rootfs_a_start=$((boot_size))
rootfs_a_end=$((rootfs_a_start+rootfs_size))
rootfs_b_start=$((rootfs_a_end))
rootfs_b_end=$((rootfs_b_start+rootfs_size))
swap_start=$((rootfs_b_end))
...
echo "Rootfs A partition size: $rootfs_size MB ($rootfs_a)"
echo "Rootfs B partition size: $rootfs_size MB ($rootfs_b)"
...
echo "Creating rootfs partition on $rootfs_a"
parted ${device} mkpart root ext4 $rootfs_a_start $rootfs_a_end

echo "Creating rootfs partition on $rootfs_b"
...
C=0
while [ $C -ne 3 ] && [ ! -e $bootfs  -o ! -e $rootfs_a -o ! -e $rootfs_b -o ! -e $swap ]; do
    C=$(( C + 1 ))
    sleep 1
done
...
if [ -d /run/media/$1/loader ]; then
    rootuuid=$(blkid -o value -s PARTUUID ${rootfs_a})
